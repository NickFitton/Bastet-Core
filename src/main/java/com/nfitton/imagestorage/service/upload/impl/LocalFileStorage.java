...
public class LocalFileStorage implements FileUploadService {
  ...
  @Override public Mono<Void> uploadFile(FilePart file, UUID imageId) {
    LOGGER.info(file.headers().toString());
    return file.transferTo(new File(path + imageId + ".jpg"));
  }

  @Override public Flux<byte[]> downloadFile(UUID imageId) {
    Path filePath = Paths.get(path + imageId.toString() + ".jpg");
    ByteBuffer buffer = ByteBuffer.allocate(8192);

    return Flux.using(() -> AsynchronousFileChannel.open(filePath, StandardOpenOption.READ),
                   ...
                   new CompletionHandler<Integer, CompletableFuture<Optional<byte[]>>>() {
                     @Override public void completed(
                       ...
                       if (read == -1) {
                         ...
                         LOGGER.trace("Finished reading chunk of size={} at position={}",
                                      read,
                                      position);
